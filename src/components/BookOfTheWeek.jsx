...
const BookOfTheWeek = () => {
  ...
  const featuredBook = {
    ...
    genre: "Dark Fantasy",
    rating: 4.9,
    summary: "A ruthless cultivator defies heaven, morality, and fate itself in his pursuit of eternal freedom and immortality.",
  ...
  return (
    <section className="py-12 bg-gradient-to-r from-teal-600 to-teal-800 text-white mb-20 rounded-sm">
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="flex flex-col md:flex-row gap-8 items-start">
            ...
            <div className="w-full md:w-2/5 flex justify-center">
              <div className="relative group">
                <img
                  ...
                  onError={(e) =>
                    (e.target.src = "https://via.placeholder.com/400x600?text=Reverend+Insanity")
                  }
                />
                <div className="absolute -top-3 -right-3 bg-yellow-400 text-teal-900 font-bold px-4 py-1 rounded-full text-sm shadow-lg">
                  This Week
                </div>
              </div>
            </div>
            ...
            <div className="w-full md:w-3/5 flex flex-col h-full">
              <div>
                <h3 className="text-2xl md:text-3xl font-bold mb-1">{featuredBook.title}</h3>
                <p className="text-lg text-teal-100 mb-3">by {featuredBook.author}</p>
              </div>
              ...
              <div className="flex items-center gap-1 text-yellow-300 mb-4">
                {[...Array(5)].map((_, i) => (
                  <svg
                    key={i}
                    className={`w-5 h-5 ${i < Math.floor(rating) ? "fill-current" : "fill-gray-400"}`}
                    viewBox="0 0 20 20"
                  >
              ...
              >
                <p className="whitespace-pre-line">{featuredBook.description}</p>
              </div>

              <div className="flex flex-col sm:flex-row gap-3 mt-6">
                <Link
                  to={`/book-details/${id}`}
                  className="px-6 py-2.5 bg-white text-teal-700 font-semibold rounded-lg transition shadow text-center border-2 border-white hover:bg-teal-700 hover:text-white"
                >
                  Read Now
                </Link>
                <button onClick={handleWishlist} className="px-6 py-2.5 border-2 border-white text-white font-semibold rounded-lg hover:bg-white hover:text-teal-700 transition">
                  Add to Wishlist
                </button>
              </div>
            </div>
